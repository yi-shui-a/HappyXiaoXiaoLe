...
public class GameAdventureScene {
    ...
    private boolean paint() throws RuntimeException { //绘制游戏场景
        ...
        //如果通关，弹出游戏通关窗口
        if ((result = cardList.getModelAdventure().passOrNot()) == 3) {
            gameSucceed(3);
        } else if ((result = cardList.getModelAdventure().passOrNot()) == 2) {
            gameSucceed(2);
        } else if ((result = cardList.getModelAdventure().passOrNot()) == 1) {
            gameSucceed(1);
        }
        //如果条件不满足，步数耗尽，弹出游戏失败窗口
        if (cardList.getModelAdventure().getStepRemain() == 0) {
            result = 0;
            gameFail();
        } else {
            cardList.levelImage[level - 1].paint(graphicsContext);
            cardList.step[step - 1].paint(graphicsContext);
            for (int i = 0; i < cardList.getModelAdventure().getRemoveSort().length; i++)
                cardList.goals[i].paint(graphicsContext);
        }
        graphicsContext.fillText("" + cardList.getModelAdventure().getScore(), 204, 247);
        for (int i = 0; i < cardList.getModelAdventure().getRemoveSort().length; i++)
            graphicsContext.fillText("X" + Math.max(cardList.getModelAdventure().getBlockRemainNum()[i],0), 140 + i * 90, 85 + i * 60);
        if (cardList.x1 != -1 && cardList.x2 != -1) {
            if (flagSwapMove == false)
                flagSwapMove = cardList.swap(moveRight, moveDown);
            if (flagSwapMove == true) {
                swapReturn = cardList.swap2Adventure(moveRight, moveDown);
                step = cardList.getModelAdventure().decreaseStepRemain();
                if (swapReturn != -2) {
                    moveRight = 0;
                    moveDown = 0;
                    flagSwapMove = false;
        ...
        if (dropSign) {
            cardList.map = cardList.getModelAdventure().getMapDropChecked();
            cardList.initCard();
        ...
        if (dropFinish) {
            dropSign2 = false;
            cardList.setMap(cardList.getModelAdventure().getMapAfter());
            repaint1 = true;
            cardList.initCard();
            cardsPaint();
            return false;
    ...
    private boolean paintReview() throws RuntimeException {
        ...
        if (isPaintReview1 == true) {
            paintJudge1 = cardList.getModelAdventure().getMapJudge();
        ...
        if (dropFinish) {
            cardList.setMap(cardList.getModelAdventure().getMapAfter());
            cardList.initCard();
            System.out.println("当前分数：" + cardList.getModelAdventure().getScore());
            cardsPaint();
            for (int i = 0; i < cardList.getModelAdventure().getRemoveSort().length; i++)
                cardList.goals[i].paint(graphicsContext);
            try {
    ...
    public void init(Stage stage, int level1) {
        ...
        running = true;
        initSprite();
        step = cardList.getModelAdventure().getStepRemain();
        for (int i = 0; i < cardList.goals.length; i++) {
            if (i == 0)
                cardList.goals[i] = new Goal(38, 28, cardList.getModelAdventure().getRemoveBlock1()[0]);
            if (i == 1)
                cardList.goals[i] = new Goal(130, 90, cardList.getModelAdventure().getRemoveBlock2()[0]);
        }
        refresh.start();
    }
    ...
    private void initSprite() {
        cardList.getModelAdventure().setLevel(level);
        cardList.initAdventureMap();
        cardList.initCard();
        cardList.initChoice();
    ...
    public void clear(Stage stage) {
        ...
        cardList.setModelAdventure(null);
        ;
    }
    ...
    public void setStage() {
        ...
        leave.setOnMouseClicked(event -> {
            ...
            Director.getInstance().adventureOver();
    ...
    public void gameSucceed(int i) {
        ...
        if (Director.getInstance().getUser().getAdventureLevel() == level) {
            Director.getInstance().getUser().setAdventureLevel(level + 1);
        }
        if(level == 20) {
        ...
        next.setOnMouseClicked(event -> {
            ...
            if(level == 20){
                ...
                Director.getInstance().adventureStart(level + 1);
        ...
        score.setText(cardList.getModelAdventure().getScore() + "");
        ...
        PersonDirectory.getInstance().writeAdventure(cardList.getModelAdventure().toString(result));
        PersonDirectory.getInstance().readPersonAdventure(Director.getInstance().getUser().getId());
        int sum = 0;
        for (int j = 0; j < 20; j++) {
            sum += Director.getInstance().getUser().getAdventureData()[j][1];
        }
        if(sum == 60 && Director.getInstance().getUser().getAchievement() /100%10 != 1){
            Director.getInstance().getUser().setAchievement(
                    Director.getInstance().getUser().getAchievement() + 100);
        }
